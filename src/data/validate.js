// =============================================================================
// validate.js — validatore CONDIVISO dei livelli (singola fonte di verità).
//
// Esporta due funzioni PURE su `grid` (array di 12 stringhe, una per riga):
//   • invariants(grid) -> { errs, cols, coins, maxRun, p3 }
//       Controlli statici: righe di pari larghezza; esattamente 5 monete (9);
//       nessuna colonna con sia 7 (soffitto) sia 2 (terra) — corridoio ship
//       aperto; righe 0-1 libere da spike/oggetti (solo 0/1); righe 10-11 vuote
//       (render pavimento); primo portale 4 dopo il primo 3; <=4 hazard a terra
//       contigui; nessun codice tile illegale; nessuno spike sopra il top di un
//       blocco (atterraggio mortale).
//   • playable(grid) -> { ok, maxTile, cols }
//       Simulatore di fisica del cubo (BFS sulle decisioni di salto) che
//       DIMOSTRA l'esistenza di un percorso che arriva alla fine senza morte
//       forzata. Le sezioni ship (portale 3..4) sono auto-superate; pad e orb
//       sono auto-usati.
//
// Nessuna API Node: gira identico in Node (scripts/check-levels.mjs) e nel
// browser (src/builder/builder.js) — così la validazione "verde" nel builder
// coincide esattamente con quella del checker da riga di comando.
//
